  ...
  <div>
    <div class="search-term">
      <el-form :inline="true" :model="searchInfo" class="demo-form-inline">
        ...
        <el-form-item>
          <el-button @click="openDialog" type="primary">新增分支机构</el-button>
        </el-form-item>
        <el-form-item>
    ...
    <el-table-column label="状态" prop="status" width="120">
      <template slot-scope="scope">
        <el-tag type="success" v-if="scope.row.status">启动</el-tag>
...
import {
    createBranch_office,
    deleteBranch_office,
    deleteBranch_officeByIds,
    updateBranch_office,
    findBranch_office,
    getBranch_officeList
...
export default {
  ...
  methods: {
      ...
      async onDelete() {
        ...
        const res = await deleteBranch_officeByIds({ ids })
        if (res.code == 0) {
          this.$message({
            type: 'success',
            message: '删除成功'
    ...
    async updateBranch_office(row) {
      const res = await findBranch_office({ ID: row.ID });
      this.type = "update";
      if (res.code == 0) {
    ...
    async deleteBranch_office(row) {
      const res = await deleteBranch_office({ ID: row.ID });
      if (res.code == 0) {
        this.$message({
          type: "success",
          message: "删除成功"
